...
class IIR_Filters:
    ...
    def highpass_filter(signal_data, sample_rate, filter_order, freq):
        ...
        try:
            filtered_signal_data = signal.lfilter(*signal.butter(filter_order, freq, 'highpass', fs=sample_rate), signal_data)
    ...
    def lowpass_filter(signal_data, sample_rate, filter_order, freq):
        ...
        try:
            filtered_signal_data = signal.lfilter(*signal.butter(filter_order, freq, 'lowpass', fs=sample_rate), signal_data)
            return filtered_signal_data
        except ValueError as e:
    ...
    def bandpass_filter(signal_data, sample_rate, filter_order, freqmin, freqmax):
        ...
        try:
            filtered_signal_data = signal.lfilter(*signal.butter(filter_order, [freqmin, freqmax], 'bandpass', fs=sample_rate), signal_data)
            return filtered_signal_data
        except ValueError as e:
    ...
    def cheby2_highpass_filter(signal_data, sample_rate, filter_order, freq, ripple):
        ...
        """
        try:
            filtered_signal_data = signal.lfilter(*signal.cheby2(filter_order, ripple, freq, 'highpass', fs=sample_rate), signal_data)
            return filtered_signal_data
    ...
    def cheby2_lowpass_filter(signal_data, sample_rate, filter_order, freq, ripple):
        ...
        """
        try:
            filtered_signal_data = signal.lfilter(*signal.cheby2(filter_order, ripple, freq, 'lowpass', fs=sample_rate), signal_data)
            return filtered_signal_data
    ...
    def cheby2_bandpass_filter(signal_data, sample_rate, filter_order, freqmin, freqmax, ripple):
        ...
        """
        try:
            filtered_signal_data = signal.lfilter(*signal.cheby2(filter_order, ripple, [freqmin, freqmax], 'bandpass', fs=sample_rate), signal_data)
            return filtered_signal_data
...
class FIR_Filters:
    ...
    def lowpass_filter(signal_data, cutoff_freq, sample_rate, filter_order, window='hamming'):
        ...
        try:
            ...
            taps = signal.firwin(filter_order, normalized_cutoff, window=window)
    ...
    def highpass_filter(signal_data, cutoff_freq, sample_rate, filter_order, window='hamming'):
        ...
        try:
            ...
            taps = signal.firwin(filter_order, normalized_cutoff, pass_zero=False, window=window)
            filtered_signal_data = signal.lfilter(taps, 1.0, signal_data)
            return filtered_signal_data
        except ValueError as e:
    ...
    def bandpass_filter(signal_data, freqmin, freqmax, sample_rate, filter_order, window='hamming'):
        ...
        try:
            ...
            taps = signal.firwin(filter_order, [low, high], pass_zero=False, window=window)
            filtered_signal_data = signal.lfilter(taps, 1.0, signal_data)
            return filtered_signal_data
        except ValueError as e:
    ...
    @staticmethod
    def kaiser_bessel_filter(signal_data, cutoff_freq, sample_rate, filter_order, beta):
        """
        ...
        """
        try:
            filtered_signal_data = signal.lfilter(*signal.firwin(filter_order, cutoff_freq, fs=sample_rate, window=('kaiser', beta)), signal_data)
            return filtered_signal_data
    ...
    def zero_phase_bandpass_filter(signal_data, freqmin, freqmax, sample_rate, filter_order):
        ...
        try:
            ...
            low = freqmin / nyquist
            high = freqmax / nyquist
            taps = signal.firwin(filter_order, [low, high], pass_zero=False, fs=sample_rate, window='blackmanharris')
            
            # Apply the filter twice to achieve zero phase
            filtered_signal_data = signal.filtfilt(taps, 1.0, signal_data)
    ...
    def wavelet_filter(signal_data, wavelet_type, level=5):
        ...
        try:
            ...
            # Example: coeffs[1:] = [c * 0.5 for c in coeffs[1:]]  # Attenuate high-frequency components
